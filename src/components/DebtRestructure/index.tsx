...
export default function DebtRestructure() {
  ...
  const rollSimulation = useMemo(() => {
    return simulateMinimumRoll(debts, incomeConfig.availableForRepayment, newBorrowRate, rollMonths);
  }, [debts, incomeConfig.availableForRepayment, newBorrowRate, rollMonths]);

  const consolidationResult = useMemo(() => {
    return calculateDebtConsolidation(debts, consolidationRate, consolidationTerm, incomeConfig.availableForRepayment);
  }, [debts, consolidationRate, consolidationTerm, incomeConfig.availableForRepayment]);

  const transferResult = useMemo(() => {
    return analyzeBalanceTransfer(highRateDebts, transferFeeRate, transferPromoRate, transferPromoMonths, transferNormalRate);
  ...
  const rollDetailColumns = [
    ...
    { title: '最低还款总额', dataIndex: 'minPaymentTotal', key: 'minPaymentTotal', render: (v: number) => `¥${formatMoney(v)}` },
  ...
  return (
    <div>
      <PageHeader
        title="债务重组策略"
        subtitle="模拟各种债务处理策略，包括借新还旧、余额转移等方案，帮助你选择最优路径"
      />
      ...
      <Tabs activeKey={activeTab} onChange={(key) => setActiveTab(key as RestructureType)}>
        <TabPane
          tab={<span><RocketOutlined /> 最低还款滚动</span>}
          key="minimum_roll"
        >
          <Card size="small" title={<span style={{ fontSize: FONT.h2, fontWeight: 600 }}>参数设置</span>} style={{ marginBottom: SPACING.lg }}>
            <Row gutter={16}>
              <Col xs={24} sm={8}>
                <Form.Item label="模拟月数">
                  <InputNumber style={{ width: '100%' }} min={1} max={60} value={rollMonths} onChange={(v) => setRollMonths(v ?? 12)} />
                </Form.Item>
              </Col>
              <Col xs={24} sm={8}>
                <Form.Item label="新借款年利率（%）">
                  <InputNumber style={{ width: '100%' }} min={0} max={50} step={0.5} value={newBorrowRate} onChange={(v) => setNewBorrowRate(v ?? 18)} />
                </Form.Item>
              </Col>
              <Col xs={24} sm={8}>
                <Form.Item label="每月可还款额（元）">
                  <InputNumber style={{ width: '100%' }} value={incomeConfig.availableForRepayment} disabled />
                </Form.Item>
              </Col>
            </Row>
          </Card>
          ...
          <Row gutter={[SPACING.lg, SPACING.lg]} style={{ marginBottom: SPACING.lg }}>
            <Col xs={24} sm={12} md={6}>
              <StatisticCard title="初始总债务" value={totalDebt} precision={2} prefix="¥" color={COLORS.danger} />
            </Col>
            <Col xs={24} sm={12} md={6}>
              <StatisticCard title="最终总债务" value={rollSimulation.finalTotalDebt} precision={2} prefix="¥" color={COLORS.danger} />
            </Col>
            <Col xs={24} sm={12} md={6}>
              <StatisticCard title="债务增长" value={rollSimulation.totalDebtGrowth} precision={2} prefix="¥" color={COLORS.orange} />
            </Col>
            <Col xs={24} sm={12} md={6}>
              <StatisticCard title="累计新借款" value={rollSimulation.newBorrowTotal} precision={2} prefix="¥" color={COLORS.warning} />
            </Col>
          </Row>
          ...
          <Card size="small" title={<span style={{ fontSize: FONT.h2, fontWeight: 600 }}>债务变化趋势</span>} style={{ marginBottom: SPACING.lg }}>
            <ReactECharts option={rollChartOption} style={{ height: 300 }} notMerge={true} />
          </Card>

          <Card size="small" title={<span style={{ fontSize: FONT.h2, fontWeight: 600 }}>月度明细</span>}>
            <Table columns={rollDetailColumns} dataSource={rollSimulation.details} rowKey="month" pagination={false} size="small" />
          </Card>
        </TabPane>

        <TabPane
          tab={<span><SwapOutlined /> 余额转移</span>}
        ...
        >
            ...
            <Row gutter={16}>
              <Col xs={24} sm={8}>
                <Form.Item label="转账手续费率（%）">
                  <InputNumber style={{ width: '100%' }} min={0} max={10} step={0.5} value={transferFeeRate} onChange={(v) => setTransferFeeRate(v ?? 3)} />
                </Form.Item>
              </Col>
              <Col xs={24} sm={8}>
                <Form.Item label="促销期利率（%）">
                  <InputNumber style={{ width: '100%' }} min={0} max={20} step={0.5} value={transferPromoRate} onChange={(v) => setTransferPromoRate(v ?? 0)} />
                </Form.Item>
              </Col>
              <Col xs={24} sm={8}>
                <Form.Item label="促销期（月）">
                  <InputNumber style={{ width: '100%' }} min={1} max={24} value={transferPromoMonths} onChange={(v) => setTransferPromoMonths(v ?? 12)} />
                </Form.Item>
              </Col>
            </Row>
            <Row gutter={16}>
              <Col xs={24} sm={12}>
                <Form.Item label="促销后年利率（%）">
                  <InputNumber style={{ width: '100%' }} min={0} max={50} step={0.5} value={transferNormalRate} onChange={(v) => setTransferNormalRate(v ?? 18)} />
                </Form.Item>
              </Col>
              <Col xs={24} sm={12}>
                <Form.Item
          ...
          {highRateDebts.length === 0 ? (
            ...
            <>
              <Row gutter={[SPACING.lg, SPACING.lg]} style={{ marginBottom: SPACING.lg }}>
                <Col xs={24} sm={12} md={6}>
                  <StatisticCard title="转账手续费" value={transferResult.transferFee} precision={2} prefix="¥" color={COLORS.danger} />
                </Col>
                <Col xs={24} sm={12} md={6}>
                  <StatisticCard title="促销期利息" value={transferResult.promoInterest} precision={2} prefix="¥" color={COLORS.warning} />
                </Col>
                <Col xs={24} sm={12} md={6}>
                  <StatisticCard title="原方案利息" value={transferResult.normalPlanInterest} precision={2} prefix="¥" color={COLORS.orange} />
                </Col>
                <Col xs={24} sm={12} md={6}>
                  <StatisticCard title="净节省" value={transferResult.totalSaving} precision={2} prefix="¥" color={transferResult.totalSaving > 0 ? COLORS.success : COLORS.danger} />
                </Col>
              </Row>
              ...
              <Card size="small" title={<span style={{ fontSize: FONT.h2, fontWeight: 600 }}>回本分析</span>}>
                <Space direction="vertical" style={{ width: '100%' }}>
                  <div>
                    ...
                    <Progress
                      percent={transferResult.breakEvenMonth > 0 ? Math.min(100, (transferPromoMonths / transferResult.breakEvenMonth) * 100) : 100}
                      showInfo={false}
                      strokeColor={COLORS.warning}
                      size="small"
                    />
                  </div>
                  <p style={{ margin: 0, fontSize: FONT.bodySmall, color: COLORS.textSecondary }}>
        ...
        <TabPane
          tab={<span><BankOutlined /> 债务整合</span>}
        ...
        >
            ...
            <Row gutter={16}>
              <Col xs={24} sm={8}>
                <Form.Item label="新贷款年利率（%）">
                  <InputNumber style={{ width: '100%' }} min={0} max={30} step={0.5} value={consolidationRate} onChange={(v) => setConsolidationRate(v ?? 12)} />
                </Form.Item>
              </Col>
              <Col xs={24} sm={8}>
                <Form.Item label="贷款期限（月）">
                  <InputNumber style={{ width: '100%' }} min={6} max={120} value={consolidationTerm} onChange={(v) => setConsolidationTerm(v ?? 36)} />
                </Form.Item>
              </Col>
              <Col xs={24} sm={8}>
                <Form.Item label="整合债务总额（元）">
                  <InputNumber style={{ width: '100%' }} value={totalDebt} disabled prefix="¥" />
                </Form.Item>
              </Col>
            </Row>
          </Card>
          ...
          <Row gutter={[SPACING.lg, SPACING.lg]} style={{ marginBottom: SPACING.lg }}>
            <Col xs={24} sm={12} md={6}>
              <StatisticCard title="新月供" value={consolidationResult.newMonthlyPayment} precision={2} prefix="¥" color={COLORS.primary} />
            </Col>
            <Col xs={24} sm={12} md={6}>
              <StatisticCard title="原方案总利息" value={consolidationResult.totalInterestOld} precision={2} prefix="¥" color={COLORS.orange} />
            </Col>
            <Col xs={24} sm={12} md={6}>
              <StatisticCard title="新方案总利息" value={consolidationResult.totalInterestNew} precision={2} prefix="¥" color={COLORS.warning} />
            </Col>
            <Col xs={24} sm={12} md={6}>
              <StatisticCard title="利息节省" value={consolidationResult.interestSaving} precision={2} prefix="¥" color={consolidationResult.interestSaving > 0 ? COLORS.success : COLORS.danger} />
            </Col>
          </Row>
          ...
          <Card size="small" title={<span style={{ fontSize: FONT.h2, fontWeight: 600 }}>方案对比</span>}>
            <Row gutter={16}>
              <Col span={12}>
                <h4 style={{ color: COLORS.danger, fontSize: FONT.h3 }}>当前方案（雪崩法）</h4>
                <p style={{ fontSize: FONT.bodySmall }}>总利息：¥{formatMoney(consolidationResult.totalInterestOld)}</p>
                <p style={{ fontSize: FONT.bodySmall }}>月均还款：¥{formatMoney(consolidationResult.monthlySaving + consolidationResult.newMonthlyPayment)}</p>
              </Col>
              <Col span={12}>
                <h4 style={{ color: COLORS.success, fontSize: FONT.h3 }}>整合后方案</h4>
                <p style={{ fontSize: FONT.bodySmall }}>总利息：¥{formatMoney(consolidationResult.totalInterestNew)}</p>
                <p style={{ fontSize: FONT.bodySmall }}>每月还款：¥{formatMoney(consolidationResult.newMonthlyPayment)}</p>
              </Col>
            </Row>
            <div style={{ marginTop: SPACING.md, padding: SPACING.md, background: '#f6ffed', borderRadius: 6 }}>
              <p style={{ margin: 0, fontSize: FONT.body }}>
                <strong>节省：</strong>
                <span style={{ color: COLORS.success, fontSize: FONT.h1, fontWeight: 600 }}>
                  ¥{formatMoney(consolidationResult.interestSaving)}
                </span>
                <span style={{ marginLeft: SPACING.lg, fontSize: FONT.bodySmall }}>
        ...
        <TabPane
          tab={<span><ThunderboltOutlined /> 借新还旧</span>}
          key="borrow_new"
        >
          <Card size="small" title={<span style={{ fontSize: FONT.h2, fontWeight: 600 }}>借新还旧策略说明</span>} style={{ marginBottom: SPACING.lg }}>
            <p style={{ fontSize: FONT.bodySmall, color: COLORS.textSecondary }}>
          ...
          <Card size="small" title={<span style={{ fontSize: FONT.h2, fontWeight: 600 }}>常见借新还旧场景</span>}>
            <Row gutter={[SPACING.lg, SPACING.lg]}>
              <Col xs={24} sm={12}>
                <Card size="small" type="inner" title="信用卡还贷款">
                  <p style={{ color: COLORS.textSecondary, fontSize: FONT.bodySmall }}>
                    用信用卡套现偿还贷款到期部分。信用卡取现手续费约1-3%，且无免息期，日息万分之五（年化18.25%）。
                  </p>
              ...
              <Col xs={24} sm={12}>
                <Card size="small" type="inner" title="网贷还信用卡">
                  <p style={{ color: COLORS.textSecondary, fontSize: FONT.bodySmall }}>
                    申请新的网贷来还信用卡账单。网贷利率通常较高（15-36%），但可能比信用卡滞纳金划算。
                  </p>
              ...
              <Col xs={24} sm={12}>
                <Card size="small" type="inner" title="账单分期">
                  <p style={{ color: COLORS.textSecondary, fontSize: FONT.bodySmall }}>
                    将信用卡账单分期，降低月供。分期手续费率通常在0.6-0.9%/月，实际年化约13-20%。
                  </p>
              ...
              <Col xs={24} sm={12}>
                <Card size="small" type="inner" title="最低还款">
                  <p style={{ color: COLORS.textSecondary, fontSize: FONT.bodySmall }}>
                    只还信用卡最低还款额（约10%）。剩余部分按日计息，年化约18%，且全额计息。
                  </p>
                  <Tag color="orange">中风险</Tag>
                </Card>
              </Col>
